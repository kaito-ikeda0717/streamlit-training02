...
def monthly_drink_sales():
    ...
    #販売月のセレクトボックスを作成
    selected_month = st.selectbox("月を選んでください",
                                drink_data.columns.values.tolist(),
                                key="drink")

    #読み込んだエクセルデータを辞書型に変更
    drink_data_dict = drink_data.to_dict()
    #辞書に変換されたドリンクデータから、選択された月のデータフレームを作成
    data = pd.DataFrame({
    ...
    st.subheader(f"{selected_month}のドリンク販売実績")
...
def monthly_meat_sales():
    ...
    #辞書に変換された肉のデータから、選択されたデータフレームを作成
    data = pd.DataFrame({"メニュー": meat_data_dict[select_month].keys(),
...
def monthly_sidemenu_sales():
    ...
    #辞書に変換されたサイドメニューのデータから、選択された月のデータフレームを作成
    data = pd.DataFrame({"メニュー": sidemenu_data_dict[select_month].keys(),
