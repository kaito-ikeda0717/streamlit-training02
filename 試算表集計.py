...
expander2 = st.expander("・賃借対照表 ")
expander2.write("賃借対照表は「決算日時点の企業の財務状態」を表します。")
...
#行と列を入れ替える
transposed_pl_data = pl_data.T
#「合計」を除くカラムを取得
transposed_pl_data = transposed_pl_data.drop(index=["合計"])
#マルチセレクト
muliselected_account = st.multiselect("項目を選んでください",
                                      ["売上高", "売上原価", "売上総利益", "販賣費・一般管理費計", "営業利益"],
                                      ["売上高"])
st.line_chart(transposed_pl_data[muliselected_account])
